...
class Talk_To_Motive():
    def __init__(self):
        self.motive_addr = "192.168.3.39"
        self.motive_port = 10008
        ...
        self.OBS_DETECT_DISTANCE = 250 # 机器人检测障碍物的范围
        self.SAFE_SELF_TARGET = 300
        self.target_id = 0
    ...
    def Epuck_rotation(self,angle):
        yaw_before_rotation = self.rigidbody["yaw"]
        print("yaw_before_rotation:", yaw_before_rotation)
        yaw_after_rotation = self.caclul_angle(yaw_before_rotation, angle)
        self.epuck.Update_Rotation(angle)
        time.sleep(0.5)  # 需要停顿一下接收动捕的新坐标
        error_angle = self.caclul_angle(yaw_after_rotation, -1 * self.rigidbody["yaw"])
        while not -5 < error_angle < 5:
            self.epuck.Update_Rotation(error_angle)
            time.sleep(0.5)  # 需要停顿一下接收动捕的新坐标
            error_angle = self.caclul_angle(yaw_after_rotation, -1 * self.rigidbody["yaw"])

        print("finally yaw:", self.rigidbody["yaw"])
        time.sleep(0.5)
    ...
    def Go_with_C_grad(self):  # 根据浓度走
        while True:
            ...
            # print(this_time_target)
            # print(theta)
            # while this_time_rigidbody["yaw"] + 10 < theta < this_time_rigidbody["yaw"] - 10:
            #     turn_angel = self.caclul_angle(theta, (-1) * this_time_rigidbody["yaw"])
            ...
            if self.Cal_self_target_distance(this_time_rigidbody,
                                             this_time_target) > self.SAFE_SELF_TARGET:  # 机器人没到目标附近的阈值
                self.epuck.Set_RGB_LEDS(0, 99, 0, 0)
                ## 计算目标到所有障碍物的距离
                if this_time_obss != []:  # 程序一开始可能还没来得及接收到刚体的数据
                    ...
                    self.Epuck_rotation(predict_c_angel[best_choice])
                ...
                print(this_time_target)
                print(theta)
                while not this_time_rigidbody["yaw"] - 20 < theta < this_time_rigidbody["yaw"] + 20:
                    turn_angel = self.caclul_angle(theta, (-1) * this_time_rigidbody["yaw"])
                    self.Epuck_rotation(turn_angel)
                    print(this_time_rigidbody)
                    print(this_time_target)
                    print(theta)
                    print("turn_angel:",turn_angel)
    ...
    def Random_motion(self):
        ...
        self.Epuck_rotation(random_angel)
    ...
    def Search_target_by_cam(self):
        ...
        if first_detected_target == second_detected_target:
            self.target_id = first_detected_target["ID"]
        else:
            ...
            if first_detected_target_distance > second_detected_target_distance:
                self.target_id = second_detected_target["ID"]
            else:
                self.target_id = first_detected_target["ID"]
        print("chosen target:",self.target_id)
